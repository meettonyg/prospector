...
/**
 * Class Interview_Finder_Ajax_Handler
 *
...
class Interview_Finder_Ajax_Handler {
     ...
     * @var string
     */
    const NONCE_SEARCH = 'interview_finder_search';
     ...
     * @var string
     */
    const NONCE_IMPORT = 'interview_finder_import';

    /**
     * Singleton instance.
     *
     * @var Interview_Finder_Ajax_Handler|null
     */
    private static ?Interview_Finder_Ajax_Handler $instance = null;
     ...
     *
     * @return Interview_Finder_Ajax_Handler
     */
    public static function get_instance(): Interview_Finder_Ajax_Handler {
    ...
    public function handle_search(): void {
        ...
        if ( ! $this->verify_nonce( self::NONCE_SEARCH ) ) {
            wp_send_json_error( [
                'message' => __( 'Security verification failed. Please refresh the page and try again.', 'interview-finder' ),
            ] );
            return;
        }
        ...
        if ( ! isset( $_POST['search_term'] ) || ! isset( $_POST['search_type'] ) ) {
            wp_send_json_error( [
                'message' => __( 'Missing required parameters.', 'interview-finder' ),
            ] );
            return;
        }
        ...
        if ( ! $ghl_id && ! $user_id ) {
            wp_send_json_error( [
                'message' => __( 'Error: Required user data not found.', 'interview-finder' ),
            ] );
            return;
        }
        ...
        if ( $search_cap > 0 && $search_count >= $search_cap ) {
            wp_send_json_error( [
                'message' => __( 'You have reached the maximum number of searches allowed for your plan.', 'interview-finder' ),
            ] );
            return;
        }
        ...
        try {
            $renderer = Interview_Finder_Renderer::get_instance();
            $results_html = '';

            if ( 'byadvancedepisode' === $search_type ) {
                $taddy_api = Interview_Finder_API_Taddy::get_instance();
                ...
                // PodcastIndex search
                $pi_api = Interview_Finder_API_PodcastIndex::get_instance();
            ...
            wp_send_json_error( [
                'message' => __( 'An error occurred during the search.', 'interview-finder' ),
    ...
    public function handle_import(): void {
        ...
        if ( ! $this->verify_nonce( self::NONCE_IMPORT ) ) {
            $error_html = '<div class="import-message error" id="import-message-container-error">'
                ...
                . '<span class="message-text">' . esc_html__( 'Security verification failed. Please refresh the page.', 'interview-finder' ) . '</span>'
                . '</div></div>';
        ...
        if ( ! isset( $_POST['podcasts'] ) || ! is_array( $_POST['podcasts'] ) ) {
            $error_html = '<div class="import-message error" id="import-message-container-error">'
                . '<div class="message-content">'
                . '<i class="fas fa-exclamation-triangle message-icon"></i>'
                . '<span class="message-text">' . esc_html__( 'Error! No podcasts were selected for import.', 'interview-finder' ) . '</span>'
                . '</div></div>';
        ...
        $form_handler = Interview_Finder_Form_Handler::get_instance();
