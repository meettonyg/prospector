...
/**
 * Class Interview_Finder_Template_Loader
 *
...
class Interview_Finder_Template_Loader {
     ...
     * @var string
     */
    private const THEME_FOLDER = 'interview-finder';
    ...
    public function render_results( array $results, string $search_type, array $options = [] ): string {
        ...
        if ( empty( $items ) ) {
            return $this->render( 'no-results', [
                ...
                'message'     => $options['no_results_message'] ?? __( 'No results found.', 'interview-finder' ),
            ], true );
        }
    ...
    public function asset_url( string $path ): string {
        return plugins_url( $path, dirname( __DIR__ ) . '/interview-finder.php' );
    }
}
