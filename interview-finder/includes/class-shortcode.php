 ...
 *
 * Handles the [interview_finder] shortcode rendering.
 *
...
/**
 * Class Interview_Finder_Shortcode
 *
...
class Interview_Finder_Shortcode {
     ...
     * Singleton instance.
     *
     * @var Interview_Finder_Shortcode|null
     */
    private static ?Interview_Finder_Shortcode $instance = null;
     ...
     *
     * @return Interview_Finder_Shortcode
     */
    public static function get_instance(): Interview_Finder_Shortcode {
    ...
    public function init(): void {
        add_shortcode( 'interview_finder', [ $this, 'render' ] );
    }
    ...
    public function render( $atts ): string {
        ...
        if ( ! $user_id ) {
            return '<p>' . esc_html__( 'Please log in to use the Interview Finder.', 'interview-finder' ) . '</p>';
        }

        $membership = Interview_Finder_Membership::get_instance();
        ...
        if ( ! $ghl_id && ! $user_id ) {
            return '<p>' . esc_html__( 'Error: Required user data not found.', 'interview-finder' ) . '</p>';
        }
        ...
        if ( $search_cap > 0 && $search_count >= $search_cap ) {
            $output .= '<div class="tabinfo msg-template pageupcta">'
                ...
                . '<h3>' . esc_html__( 'You used up all of your searches on your current plan', 'interview-finder' ) . '</h3>'
                . '<p>' . sprintf(
                    /* translators: %s: pricing link */
                    esc_html__( 'You have reached the maximum number of searches allowed this month. %s to continue searching.', 'interview-finder' ),
                    '<a href="/pricing/">' . esc_html__( 'Upgrade your plan', 'interview-finder' ) . '</a>'
                ) . '</p>'
                . '<p><a class="btn-trial-cta" href="/pricing/" role="button">'
                . esc_html__( 'Upgrade your plan', 'interview-finder' ) . ' <i class="fas fa-lock"></i></a></p>'
                . '</div>';
        }
    ...
    private function render_tabs( string $selected ): string {
        $tabs = [
            'byperson'          => __( 'Search Episodes by Person', 'interview-finder' ),
            'bytitle'           => __( 'Search Podcasts by Title', 'interview-finder' ),
            'byadvancedpodcast' => __( 'Advanced Search Podcasts', 'interview-finder' ),
            'byadvancedepisode' => __( 'Advanced Search Episodes', 'interview-finder' ),
        ];
    ...
    private function render_search_row( string $search_term, array $settings, int $number_of_results, int $results_per_page, string $sort_order ): string {
        ...
        $output .= '<div class="left-group" style="display:flex; gap:10px; align-items:center;">'
            . '<input type="text" name="search_term" class="search-term" placeholder="' . esc_attr__( 'Enter search term', 'interview-finder' ) . '" '
            . 'value="' . esc_attr( $search_term ) . '" required style="min-width:220px;">'
            . '<input type="submit" class="search-btn" value="' . esc_attr__( 'Search', 'interview-finder' ) . '">'
            . '<button id="toggle-filters" class="filter-btn" type="button" style="display:none;">' . esc_html__( 'Filter', 'interview-finder' ) . '</button>'
            . '</div>';
        ...
        $output .= '<div id="basic-block" style="display:none;">'
            . '<label for="number_of_results" class="results-label">' . esc_html__( 'Number of Results:', 'interview-finder' ) . '</label>'
        ...
        // Results per page
        $output .= '<label for="results_per_page" class="results-label">' . esc_html__( 'Display:', 'interview-finder' ) . '</label>'
        ...
        // Sort order
        $output .= '<label for="sort_order" class="results-label">' . esc_html__( 'Sort By:', 'interview-finder' ) . '</label>'
            . '<select name="sort_order" id="sort_order" class="results-dropdown">';

        $sort_options = [
            'BEST_MATCH' => __( 'Best Match', 'interview-finder' ),
            'LATEST'     => __( 'Latest', 'interview-finder' ),
            'OLDEST'     => __( 'Oldest', 'interview-finder' ),
        ];
    ...
    private function render_filters( array $settings, string $language, string $country, string $genre, string $after_date, string $before_date, bool $is_safe_mode ): string {
        ...
        $output .= '<div class="filter-group">'
            . '<label for="after_date">' . esc_html__( 'Published After:', 'interview-finder' ) . $lock_icon . '</label>'
            . '<input type="date" name="after_date" id="after_date" value="' . esc_attr( $after_date ) . '" '
            . ( $date_disabled ? 'disabled' : '' ) . '>'
            . '</div>';

        $output .= '<div class="filter-group">'
            . '<label for="before_date">' . esc_html__( 'Published Before:', 'interview-finder' ) . $lock_icon . '</label>'
        ...
        if ( $settings['safe_mode_forced'] ) {
            $output .= '<div class="filter-group">'
                . '<p>' . esc_html__( 'Explicit content is disabled on your plan', 'interview-finder' ) . ' <span class="locked"><i class="fas fa-lock"></i></span></p>'
                . '<input type="hidden" name="isSafeMode" value="true">'
                . '</div>';
        } else {
            $checked = ! $is_safe_mode ? 'checked' : '';
            $output .= '<div class="filter-group">'
                . '<label><input type="checkbox" name="isSafeMode" ' . $checked . '> ' . esc_html__( 'Include explicit content', 'interview-finder' ) . '</label>'
                . '</div>';
        }
    ...
    private function render_filter_select( string $name, string $label, array $options, string $selected, bool $disabled ): string {
        ...
        foreach ( $options as $option ) {
            ...
            $display = 'ALL' === $option ? __( 'All', 'interview-finder' ) : ucwords( strtolower( str_replace( [ 'PODCASTSERIES_', '_' ], [ '', ' ' ], $option ) ) );
