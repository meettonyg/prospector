...
/**
 * Class Interview_Finder_Form_Handler
 *
...
class Interview_Finder_Form_Handler {
     ...
     * Singleton instance.
     *
     * @var Interview_Finder_Form_Handler|null
     */
    private static ?Interview_Finder_Form_Handler $instance = null;
     ...
     */
    private ?Interview_Finder_Settings $settings = null;

    /**
     * Get singleton instance.
     *
     * @return Interview_Finder_Form_Handler
     */
    public static function get_instance(): Interview_Finder_Form_Handler {
    ...
    public function create_entries( array $podcasts, array $post_data ): array {
        ...
        foreach ( $podcasts as $item ) {
            ...
            if ( ! $decoded || ! is_array( $decoded ) ) {
                $fail_count++;
                $failed_items[] = __( 'Invalid data format received for an item.', 'interview-finder' );
            ...
            if ( empty( $entry_data['feed_url'] ) ) {
                ...
                $failed_items[] = sprintf(
                    ...
                    __( "Podcast '%s' couldn't be imported due to missing RSS feed.", 'interview-finder' ),
            ...
            if ( class_exists( 'FrmEntry' ) ) {
                ...
                if ( ! $entry_id || is_wp_error( $entry_id ) ) {
                    ...
                    $failed_items[] = sprintf(
                        ...
                        __( "Podcast '%1\$s' failed: %2\$s", 'interview-finder' ),
                        $entry_data['podcast_title'],
                        $error_msg
                    );
                ...
                $failed_items[] = sprintf(
                    /* translators: %s: podcast name */
                    __( "Formidable Forms not available. Could not import '%s'.", 'interview-finder' ),
                    $entry_data['podcast_title']
                );
    ...
    private function generate_response_html( int $success_count, int $fail_count, int $total, array $failed_items ): string {
        ...
        if ( $fail_count === 0 && $success_count > 0 ) {
            ...
            return sprintf(
                ...
                sprintf(
                    ...
                    esc_html__( 'Success! %1$d %2$s been added to your collection.', 'interview-finder' ),
                    $success_count,
                    $success_count === 1 ? esc_html__( 'podcast has', 'interview-finder' ) : esc_html__( 'podcasts have', 'interview-finder' )
                ),
                esc_url( $tracker_link ),
                esc_html__( 'View in Interview Tracker', 'interview-finder' )
            );
        }
        ...
        if ( $fail_count > 0 ) {
            ...
            if ( $success_count > 0 ) {
                $error_summary = sprintf(
                    ...
                    esc_html__( 'Partial success! %1$d of %2$d podcasts added to your collection.', 'interview-finder' ),
                    $success_count,
                    $total
                );
            } else {
                $error_summary = sprintf(
                    /* translators: 1: count, 2: singular/plural text */
                    esc_html__( 'Error! %1$d %2$s could not be imported.', 'interview-finder' ),
                    $fail_count,
                    $fail_count === 1 ? esc_html__( 'podcast', 'interview-finder' ) : esc_html__( 'podcasts', 'interview-finder' )
                );
            }
        ...
        return '<div class="import-message error" id="import-message-container-error">'
            . '<div class="message-content">'
            . '<i class="fas fa-exclamation-triangle message-icon"></i>'
            . '<span class="message-text">' . esc_html__( 'No podcasts were processed. Please check selection.', 'interview-finder' ) . '</span>'
            . '</div>'
            . '</div>';
    }
