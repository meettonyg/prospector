...
function interview_finder_create_table() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'interview_finder';
...
register_activation_hook(__FILE__, 'interview_finder_create_table');
 ...
 * @param int|null    $user_id  The WordPress user ID (if available).
 */
function interview_finder_reset_search_cap_if_needed( $ghl_id = null, $user_id = null ) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'interview_finder';
 ...
 * }
 */
function interview_finder_get_membership_settings($user_id) {
...
function interview_finder_get_user_data($ghl_id = null, $wp_user_id = null) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'interview_finder';
...
function interview_finder_get_or_create_user_data($ghl_id = null, $wp_user_id = null) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'interview_finder';

    // Check for an existing record
    $result = interview_finder_get_user_data($ghl_id, $wp_user_id);
...
function interview_finder_update_user_data($ghl_id = null, $wp_user_id = null) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'interview_finder';

    // Retrieve or create the user record
    $user_data = interview_finder_get_or_create_user_data($ghl_id, $wp_user_id);
 ...
 */
function interview_finder_get_language_name($lang_code) {
...
function search_podcastindex($search_term, $search_type = 'byperson', $max_results = null) {
    ...
    $settings = interview_finder_get_membership_settings($user_id);
...
function display_podcast_results( $data, $search_term = '' ) {
    ...
    foreach ( $results as $index => $item ) {
        ...
        $raw_language  = interview_finder_get_language_name( $lang_code );
...
function display_taddy_api_podcast_results($response, $search_term = '') {
    ...
    if ( $results_count > 0 ) {
        ...
        foreach ( $results as $index => $podcast ) {
            ...
            $raw_name      = interview_finder_get_language_name($lang_code);
...
function handle_perform_search() {
    ...
    $user_data = interview_finder_get_or_create_user_data($ghl_id, $user_id);

    // Now perform the reset check.
    interview_finder_reset_search_cap_if_needed($ghl_id, $user_id);

    // Retrieve the user's data from the custom table.
    $user_data = interview_finder_get_user_data($ghl_id, $user_id);
    ...
    interview_finder_update_user_data($ghl_id, $user_id);

    // Fetch updated user data.
    $updated_user_data = interview_finder_get_user_data($ghl_id, $user_id);
...
function interview_finder_shortcode($atts) {
    $output = '';
    ...
    // Membership settings
    $membership = interview_finder_get_membership_settings($user_id);
...
add_shortcode('interview_finder', 'interview_finder_shortcode');

function interview_finder_styles() {
    // Only load assets on the specific interview detail page (ID: 43072)
    if ( ! is_page( 43072 ) ) {
        return;
    }
    
    wp_enqueue_style('interview-finder-styles', plugins_url('assets/styles.css', __FILE__), array(), null);
    wp_enqueue_script('jquery');
}
add_action('wp_enqueue_scripts', 'interview_finder_styles');

function interview_finder_enqueue_scripts() {
    // Only load assets on the specific interview detail page (ID: 43072)
    if ( ! is_page( 43072 ) ) {
        return;
    }
...
add_action('wp_enqueue_scripts', 'interview_finder_enqueue_scripts');
