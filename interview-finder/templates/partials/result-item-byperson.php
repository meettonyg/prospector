...
$date_published = $item['datePublished'] ?? 0;
$link = $item['link'] ?? '';
$feed_image = $item['feedImage'] ?? '';
$feed_url = $item['feedUrl'] ?? '';
$item_id = $item['id'] ?? $index;
?>
<li class="if-result-item" data-index="<?php echo esc_attr( $index ); ?>">
    ...
    <div class="if-result-item__image">
        <?php if ( $feed_image ) : ?>
            <img
                src="<?php echo esc_url( $feed_image ); ?>"
                alt=""
                loading="lazy"
                width="80"
                height="80"
    ...
    <div class="if-result-item__content" id="if-item-desc-<?php echo esc_attr( $item_id ); ?>">
        ...
        <?php if ( $feed_title ) : ?>
            <p class="if-result-item__podcast"><?php echo esc_html( $feed_title ); ?></p>
        <?php endif; ?>

        <?php if ( $date_published ) : ?>
            <time class="if-result-item__date" datetime="<?php echo esc_attr( gmdate( 'c', $date_published ) ); ?>">
                <?php echo esc_html( date_i18n( get_option( 'date_format' ), $date_published ) ); ?>
            </time>
        <?php endif; ?>
